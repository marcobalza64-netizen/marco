#!/usr/bin/env python3
"""
Monitor titoli — mercati europei in tempo reale
Nessuna libreria da installare: usa solo Python standard.

Titoli: NVIDIA, Tesla, Rheinmetall
Fonti:
- Xetra / Francoforte: API Börse Frankfurt (tempo reale)
- Tradegate: tempo reale
- USA / Yahoo: Yahoo Finance (near real-time)
"""
...
# Retrocompatibilità CLI/import vecchi
MARKETS = STOCKS["nvidia"]["markets"]
NVIDIA_ISIN = STOCKS["nvidia"]["isin"]
...
_YAHOO_COOKIE_FILE = "/tmp/stock_monitor_yf.cookies"
...
def get_stock(stock_key: str) -> dict:
    if stock_key not in STOCKS:
        raise KeyError(f"Titolo non valido: {stock_key}")
    return STOCKS[stock_key]


def get_market(stock_key: str, market_key: str) -> dict:
    stock = get_stock(stock_key)
    markets = stock["markets"]
    if market_key not in markets:
        raise KeyError(f"Mercato non valido per {stock['name']}: {market_key}")
...
def fetch_boerse(stock_key: str, market_key: str) -> Quote:
    stock = get_stock(stock_key)
...
def fetch_tradegate(stock_key: str, market_key: str) -> Quote:
    stock = get_stock(stock_key)
    market = get_market(stock_key, market_key)
    tz_name = market["tz"]
    url = f"https://www.tradegate.de/refresh.php?isin={stock['isin']}"
...
def fetch_yahoo_chart(stock_key: str, market_key: str) -> Quote:
    market = get_market(stock_key, market_key)
    symbol = market["symbol"]
    tz_name = market["tz"]
    url = (
...
def fetch_yahoo(stock_key: str, market_key: str) -> Quote:
    try:
        market = get_market(stock_key, market_key)
        symbol = market["symbol"]
        tz_name = market["tz"]
        ...
        return fetch_yahoo_chart(stock_key, market_key)


def fetch_quote(market_key: str, stock_key: str = "nvidia") -> Quote:
    market = get_market(stock_key, market_key)
    if market["source"] == "boerse":
        return fetch_boerse(stock_key, market_key)
    if market["source"] == "tradegate":
        return fetch_tradegate(stock_key, market_key)
    return fetch_yahoo(stock_key, market_key)
...
def is_market_open(market_key: str, now: datetime) -> bool:
    ...
    if market_key in {"xetra", "frankfurt", "tradegate", "yahoo"}:
...
def render(quote: Quote, stock_key: str, market_key: str, interval: int, error: Optional[str]) -> str:
    stock = get_stock(stock_key)
    market = get_market(stock_key, market_key)
    up = quote.change >= 0
    ...
    lines = [
        f"{bold}{stock['name']} — {market['name']}{reset}  {dim}({quote.symbol}){reset}",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Monitor titoli Europa in tempo reale")
    parser.add_argument(
        "-s",
        "--stock",
        choices=sorted(STOCKS.keys()),
        default="nvidia",
        help="Titolo (default: nvidia)",
    )
    parser.add_argument(
        "-m",
        "--market",
...
def main() -> int:
    ...
    try:
        get_market(args.stock, args.market)
    except KeyError as exc:
        print(exc)
        print(f"Mercati disponibili: {', '.join(STOCKS[args.stock]['markets'])}")
        return 1
    ...
    def refresh() -> None:
        ...
        try:
            last_quote = fetch_quote(args.market, args.stock)
            last_error = None
        except Exception as exc:  # noqa: BLE001
            last_error = str(exc)
        ...
        clear()
        print(render(last_quote, args.stock, args.market, args.interval, last_error))

    if args.once:
        refresh()
        return 0 if last_error is None and last_quote is not None else 1

    stock_name = STOCKS[args.stock]["name"]
    print(f"Avvio monitor {stock_name} in tempo reale…")
    try:
        while True:
            refresh()
